	...
	<div class="lefter">
        ...
        <div class="article">
            <?php
                ...
                foreach ($information->getIDInfo($_GET['infoID']) as $v):
                    ?>
                    <h3><?=$v['infoTitle']?></h3>
                    <h4>发布时间：<?=$v['infoTime']?></h4>
                    <?=$v['infoContext']?>
                    <?php
                endforeach;
            endif;
            ...
            <?php
                    ...
                    foreach ($news->getIDNews($_GET['newsID']) as $v):
                ?>
                <h3><?=$v['newsTitle']?></h3>
                <h4>发布时间：<?=$v['newsTime']?></h4>
                <?=$v['newsContext']?>
        	<?php
                    endforeach;
                    endif;
    	...
    	<div class="rightBox">
            ...
            <ul class="list_r">
                <?php foreach ($news->getNews() as $v):?>
                    <li><a href='article.php?newsID=<?=$v['newsID']?>'><?=$v['newsTitle']?> </a>
